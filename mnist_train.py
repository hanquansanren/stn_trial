...
def train(epoch):
    ...
    bg = cv2.imread('temp_img/bg.png', flags=cv2.IMREAD_GRAYSCALE)
    # bg = cv2.cvtColor(bg, cv2.COLOR_BGR2RGB)
    bg = cv2.resize(bg, (28,28), interpolation=cv2.INTER_LINEAR) 
    bg = torch.from_numpy(bg).float().cuda().unsqueeze(0).unsqueeze(0)
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        # output = model(data, bg)
        output = model(data)
        loss = F.nll_loss(output, target)
        loss.backward()
        optimizer.step()
        if batch_idx % args.log_interval == 0:  # 打印频率，每10个iteration打印一次
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                epoch, batch_idx * len(data), len(train_loader.dataset),
                100. * batch_idx / len(train_loader), loss.item()))
        if batch_idx % args.save_interval == 0: # 存储频率，每10个iteration打印一次
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type = str, default='unbounded_stn')
    ...
    args.span_range_height = args.span_range_width = args.span_range # 0.9
